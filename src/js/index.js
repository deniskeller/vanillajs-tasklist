...
function createTodo() {
  ...
  if (textarea.value) {
    ...
    ;
    db.collection("todos").doc(id).set(todo).then((textarea.value = "")).then(addToLocalStorage(todo)).then(renderList);
  }
}
...
function renderList() {
  ...
  function displayTodos() {
    const noTodo = newTodo.length ? newTodo.map(getContent).join("") : `<div class="task-empty">У вас пока нет задач</div>`;
    ...
    try {
      ...
      for (let i = 0; i < document.querySelectorAll(".task-list__item").length;

        i++) {
        document.querySelectorAll(".task-list__item__edit")[i].addEventListener("click", () => {
            document.querySelectorAll(".task-list__item__menu")[i].classList.toggle("hide");
            document.querySelectorAll(".task-list__item")[i].classList.toggle("active");
          }
    ...
    document.onclick = (e) => {
      for (let i = 0; i < document.querySelectorAll(".task-list__item").length;
        ...
        i++) {
        if (!document.querySelectorAll(".task-list__item")[i].contains(e.target)) {
          document.querySelectorAll(".task-list__item__menu")[i].classList.add("hide");
          document.querySelectorAll(".task-list__item")[i].classList.remove("active");
        }
      }
    }
...
function getContent(todo) {
  return `<div class="task-list__item"id="${todo.id}"><span>$ {
    todo.index+1
  }
  ...
  </span><div class="task-list__item__edit"></div><div class="task-list__item__menu hide"><div class="task-list__item__menu-item done-task"id="${
todo.id
}
...
function deleteTask(id) {
  db.collection("todos").doc(id).delete().then(deleteFromLocalStorage(id)).then(renderList);
}
...
function doneTask(todo) {
  ...
  ).then(updateToLocalStorage(todo)).then(done(todo));
}
